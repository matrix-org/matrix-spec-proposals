...
The delta token sits outside the scope of sessions, and hence can be used to remember data between
...
A worked example:
 ...
 - The client reappears and hits `/sync` to start a new session. The delta token takes effect and
...
The delta token will not work under the following scenarios:
 ...
 - The server does not need to remember delta tokens and the associated data forever. The server
   ...
   to the client and a new delta token being generated. This MSC recommends that servers keep bandwidth
