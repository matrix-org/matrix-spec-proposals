...
These examples are broken down into:
 - `state`: Pre-existing state on the server e.g from a database. These should be executed in the order
   ...
   configure the server into a certain state ready for sliding sync requests.
 - `request`: The sliding sync client request.
...
```json
{
  "state": {
    "create_rooms": {
      ...
      "!b:localhost": {"preset": "public_chat", "name": "Sync Test Room"},
      "!c:localhost": {"preset": "public_chat", "room_alias_name": "synctestalias"},
...
```json
{
  ...
  "new_state": {
    ...
    "rooms": {
      ...
      "!b:localhost": {
        "current_state": {
          "m.room.name": {"name":"Sync Test Room"}
        },
        "timeline": []
      },
      "!c:localhost": {
        "current_state": {
          "m.room.name": {"name":"#synctestalias:localhost"}
        },
        "timeline": []
      },
      "!d:localhost": {
