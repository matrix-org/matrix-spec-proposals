...
Similarly, we reserve a `rooms` field, which is a sticky list of room IDs. It has the same semantics
as `lists`, except it tells the extension which room subscriptions it should process (instead of sliding
windows).
