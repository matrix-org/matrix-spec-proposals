...
```json5
{
  ...
  // Sticky. Allowlist of event types which should cause the lists of rooms to be reordered.
  // By omitting types from this list, clients can ensure that uninteresting events (e.g. a
  // profile rename) do not cause a room to jump to the top of its list(s).
  // Empty or omitted `bump_event_types` fall back to the list ["m.room.message", "m.room.encrypted"].
  // NB: this is currently per-connection, not per-list.
  "bump_event_types": [ "m.room.message", "m.room.encrypted" ],
}
```
