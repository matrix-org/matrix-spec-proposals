  ...
  "lists": [
    {
      ...
      // If this key is missing entirely, no sliding window is used and all rooms for this list are returned.
      // In this particular case, the `ops` field will not be present in the response.
      "ranges": [ [0,9], [20,29] ],
...
```js
{
  ...
  "lists": [
    // List 0
    {
      ...
      "count": 1337,
      // The sliding list operations to perform. This field will be missing if the request specifies
      // no ranges.
      "ops": [
        {
      ...
      // then you could see `ops` with DELETE[4,!foo:bar], INSERT[0,!foo:bar], UPDATE[0,!foo:bar]
...
to opt-out of sliding window semantics entirely (the `ops` key just disappears)._
...
[Proxy server](https://github.com/matrix-org/sync-v3) (v0.1.0):
 - Sliding Window API:
    - [x] Operation support
