...
Clients MAY re-fetch more frequently and MAY use a shorter poll interval while presence view is open. Past an entry's `expiry`, the client cannot know whether a successor entry exists, so it MUST stop displaying the expired entry as current and MUST re-fetch to find any successors.
### Examples
#### Games
```json
{
...
## Unstable prefix
While this MSC is considered unstable, `m.rpc` SHOULD be replaced with `xyz.extera.msc4544.rpc` in all field names and type identifiers.
