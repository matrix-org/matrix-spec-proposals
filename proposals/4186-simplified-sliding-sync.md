...
the second to last event previously). If the room then drops below the threshold (and so has timeline limit of 1), and
then receives another update, the server MAY remember that it has already sent the previous 10 events and only return
the latest one.
...
| `room_name_like` | `string` | No | Filter the room name. <br/><br/>Case-insensitive partial matching e.g 'foo' matches 'abFooab'. The term 'like' is inspired by SQL 'LIKE', and the text here is similar to '%foo%'. <br/><br/> The exact matching semantics is left up to the implementation, for example a server may user full-text search rather than simple matching. |
