...
```json
{
    ...
    "bookmark_ids": [
        ...
        "chr_Hiemu5ahKo",
        "69JH5EWWFKTDQCT5N5D9LAPEMQNV7HMNUA7RSFJX2VHE-E"
    ]
}
```
...
value, it should refresh its bookmark list from the bookmark item events it has already received
via `/sync`. Client should request bookmark item events from the server only if they are missing locally.
...
The qualifying implementation uses the simple djb2-like hash algorithm with a `bmk_` prefix to
generate bookmark IDs and deduplicate bookmarks by `(room_id, event_id)`.
That scheme is a reference implementation
detail and is not required by this MSC.
...
`deleted: true` signals to all clients that this bookmark should be ignored, while preserving the
bookmark item so it can later be resurrected by setting `deleted: false` and re-adding its ID to
`bookmark_ids`. Clients should tombstone the bookmark item event rather than abandoning its prior
content, so that other clients which encounter the item event can determine that the bookmark was
explicitly removed rather than merely missing.
...
should refresh its bookmark list from the bookmark item events it already has locally. Because
clients write bookmark items before updating the index, the corresponding item changes should
already have been delivered via `/sync`.
...
The qualifying implementation also currently uses its own bookmark ID scheme. That implementation
detail is not required by this MSC and may change as the proposal evolves.
...
The reference implementation uses its own bookmark ID scheme. Clients should consider collision
risk carefully, because two different room-event pairs could map to the same account data event
type and overwrite each other. One mitigation is to check for existing bookmarks for the same
`(room_id, event_id)` and reuse their `bookmark_id`.
...
depends on the chosen bookmark ID algorithm. Implementations should consider that trade-off
carefully. Reusing another client's bookmark ID format with a different algorithm can also create inter-client clashes.
