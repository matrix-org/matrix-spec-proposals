...
This endpoint will take a required query parameter of `user_id` which will contain the MXID of the user
matched against.
...
If the user ID is [non-compliant](https://spec.matrix.org/v1.17/appendices/#historical-user-ids),
...
The server may decide that the response to this endpoint is too large, and only return
a subset of the results. In this case, the server should populate the optional field
`"next_batch"` with an
[opaque identifier](https://spec.matrix.org/v1.16/appendices/#opaque-identifiers).
The client may then supply the identifier as a `from` query parameter in a subsequent
request, along with the original `user_id`, to fetch the next batch of responses. This will
continue until the server no longer inserts `"next_batch"`, meaning there are no further
results. For example:
...
If the batch token provided by the client is deemed invalid by the server, the server
MAY return the standard error response with HTTP code 400 and `errcode`
`M_INVALID_PARAM`.

If the client supplies their own user ID in the `user_id` query parameter, the server MUST return
HTTP code 400, with `M_INVALID_PARAM` as the `errcode`. The `error` field value is up to the
implementation. A suggested value is "you cannot request rooms in common with yourself".

Batch tokens generated by this endpoint SHOULD be valid for at least 10 minutes, after which, tokens can expire.
Expired tokens SHOULD be handled similar to invalid tokens, as described above.

When a client uses batch tokens, servers MAY omit rooms that the user joined after the first token
was generated.
...
parameter for the time being. This restriction may be lifted by a future MSC.
...
The /versions endpoint MUST include a new boolean key in `unstable_features` with the name
`uk.half-shot.msc2666.query_mutual_rooms`.

Previous iterations of this MSC have used the following `unstable_features` key(s):
