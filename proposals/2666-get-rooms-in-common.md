...
continue until the server no longer inserts `"next_batch"`. The `next_batch` field follows
...
`M_UNKNOWN` as the `errcode`. The `error` field value is up to the implementation. A suggested
value is "you cannot request rooms in common with yourself".
