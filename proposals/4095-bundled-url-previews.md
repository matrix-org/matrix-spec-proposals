...
list, and SHOULD NOT display previews for URLs that aren't present in the list.

An empty `m.url_previews` array is used to indicate that the sender does not
want any URLs to be previewed.

If the `m.url_previews` field is not present at all, clients MAY fall back to
the old behavior of searching `body`.
...
Linked media (e.g. [MSC3911]) would alleviate this by removing the media when
