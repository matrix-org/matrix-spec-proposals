...
instead of having universal permissions.
...
* If a selector is in the blacklist in one rule and in the blacklist in a later rule of the same kind,
...
broken by assuming the role that is explicitly declared in the role listing is the bottom-most.
...
array will be empty to denote that users do not have permission to edit other people's messages.
Top role T has the equivalent of `["*"]`, meaning anyone may have their messages redacted by users
in the applicable role.
