...
OK problem, `m.room.server.capability` won't work -- we can't dispatch on both
state keys for member and cap name. Instead we need to have `m.room.server_capability.member`
and `m.room.user_capability.member` as an example of the membership capability.

### Ditching `m.room.member`

`m.room.member` is bad because it is part of the auth chain, lots of mess,
multiple roles for avatars (very bad).
Should just be ditched from the auth chain.
...
##### `m.room.server_capability.events` state_key: server name

same as `m.room.capability_events`.
...
if you're the last member to have a cpaability, if iti s taken away from you
it must be taken away from the server surely?

### Advnatages

you can stop servers generating joins by taking away theier capability to send
the event `m.room.user_capability.member`. But then how do we let existing
users leave?
