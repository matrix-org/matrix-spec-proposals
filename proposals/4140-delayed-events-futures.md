...
https://spec.matrix.org/v1.19/client-server-api/#transaction-identifiers),
and supports [timestamp massaging](https://spec.matrix.org/v1.19/application-service-api/#timestamp-massaging)
...
The `delay_id` is an [opaque identifier](https://spec.matrix.org/v1.19/appendices/#opaque-identifiers)
generated by the homeserver.

The homeserver MUST NOT send the event before the scheduled time (except in response to the
[`send` action](#managing-scheduled-delayed-events) introduced below) and MAY send it up to 30 seconds
after the scheduled send time.
Note: clients might find that their events are delayed further due to server load and similar conditions,
or failed to be sent due to having surpassed usual rate limits for sending events in a room as if the event had been
sent as a non-delayed event.

> **Process note**: The proposal originally did not clarify that using the `send` action
> bypasses the rule to not send a delayed event before its scheduled send time.
...
a [standard error response](https://spec.matrix.org/v1.19/client-server-api/#standard-error-response)
...
If the feature is disabled on the server via configuration, the homeserver SHOULD instead respond with HTTP 403 and
a [standard error response](https://spec.matrix.org/v1.19/client-server-api/#standard-error-response)
...
[High Volume of Messages](https://spec.matrix.org/v1.19/appendices/#threat-high-volume-of-messages) threat.
...
- `content` - Required. The content of the delayed event.
  ...
  > **Process note**: The proposal had a typo where this previously placed the entire request
  > body into `content`, including the fields communicated elsewhere in this schema.
...
https://spec.matrix.org/v1.19/appendices/#threat-high-volume-of-messages) threat where a malicious actor
...
However, this approach adds security obligations on the `delay_id`, due to it acting as a kind of access token.
For instance, it should be cryptographically secure to mitigate unauthorized access to delayed event management.
APIs must also be designed to not leak `delay_id`s, such as to server logs that track the paths of requested endpoints.

Another issue is increased difficulty of effectively rate limiting the unauthenticated management endpoints.
Without a per-user access token, the only option for identifying the source of requests would be the source IP address.
This would result in aggressive rate-limiting of an external service that manages delayed events for many users,
as all of its requests would come from the same source IP address.
This could be mitigated by rate limiting the management endpoints based on the `delay_id`.

Alternatively, instead of the delayed event management endpoints being unauthenticated,
...
Some alternative names for the `delayed_since_ts` field on the `GET` response are:

- `running_since_ts` - clearly indicates the purpose of the field, but
  no other part of this proposal uses the term `running` to describe a scheduled delayed event
- `scheduled_ts` - reuses existing terminology of `scheduled`, but
  might be misinterpreted as being the scheduled send time of the delayed event, and
  might wrongly suggest that the time is always relative to the initial scheduling, rather than the most recent restart
- `scheduled_at` - is consistent with the spec's usage of a suffix of `_at` to designate timestamp-valued fields, but
  might still be misinterpreted as the scheduled send time or as the time relative to the initial scheduling
...
[^eventId]: An event's ID is computed from its [reference hash](https://spec.matrix.org/v1.19/rooms/v11/#event-ids)
