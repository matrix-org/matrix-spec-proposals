...
It MUST be unique with respect to the server's stored delayed events and SHOULD be cryptographically secure (in the sense that it is infeasible to predict).
This way, clients MAY share the `delay_id` with external services to allow delegated management of
their delayed events through the unauthenticated management endpoints introduced
[below](#managing-scheduled-delayed-events). The cryptographic randomness of the `delay_id` mitigates
unauthorized access to the delayed event.
...
`POST /delayed_events/{delay_id}/restart` endpoint while a user is connected
