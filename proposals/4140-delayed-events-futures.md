...
The Client-Server API currently has a  [Voice over IP module](https://spec.matrix.org/v1.17/client-server-api/#voice-over-ip)
...
[`PUT /_matrix/client/v3/rooms/{roomId}/send/{eventType}/{txnId}`](https://spec.matrix.org/v1.17/client-server-api/#put_matrixclientv3roomsroomidsendeventtypetxnid)
endpoint. It also makes use of [transaction identifiers](https://spec.matrix.org/v1.17/client-server-api/#transaction-identifiers).
...
The `delay_id` is an [opaque identifier](https://spec.matrix.org/v1.17/appendices/#opaque-identifiers) generated by the server.
...
per the [pagination convention](https://spec.matrix.org/v1.17/appendices/#pagination). The homeserver can choose a suitable
page size.
...
These objects contain the following fields:
  ...
  - `error` - Optional. A Matrix error (as defined by [Standard error response](https://spec.matrix.org/v1.17/client-server-api/#standard-error-response))
...
[High Volume of Messages](https://spec.matrix.org/v1.17/appendices/#threat-high-volume-of-messages) threat where a malicious
...
The Matrix spec says that the `event_id` must use the [reference hash](https://spec.matrix.org/v1.17/rooms/v11/#event-ids)
which is [calculated from the fields](https://spec.matrix.org/v1.17/server-server-api/#calculating-the-reference-hash-for-an-event)
of an event including the `origin_server_ts` as defined in [this list](https://spec.matrix.org/v1.17/rooms/v11/#client-considerations)
...
corresponds to the [High Volume of Messages](https://spec.matrix.org/v1.17/appendices/#threat-high-volume-of-messages)
